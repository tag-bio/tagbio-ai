...
| `joins` | inner-join another table and pull specific `columns` in as implicit columns for this table's parsers. The **join table must be unique on its key** (the "one" side); it may serve **many host rows** (one-join-to-many-host is fine). Never many-join-to-one-host — if several join rows match one host row, one is taken arbitrarily; use an `other_table` for that. Reference a pulled column as `"<table_alias>.<column>"` |
| `include` | restrict which **columns** the table loads |
| `data_dictionary` | a `From,To` CSV that **renames source columns to canonical names before parsing**; **many→one is allowed** — map several files' differently-named columns onto one name so a *single* parser targets it (e.g. `visit_dt` / `enc_date` / `admit_date` all → `encounter_date`). The workhorse for unioning many same-grain files whose columns are named inconsistently. **Note:** a `data_dictionary` at the **config top level** is a *different* thing — the build's **output** dictionary TSV (`configuration-and-sources.md`); this table-level one is an *input* rename |
